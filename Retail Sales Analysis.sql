...
# Objectives:
-- Data Cleaning: Identify and remove any records with missing or null values, standardizing column names, and transforming columns data type.
...
-- 18. Write a SQL query to find the distribution of customers by age group (e.g., 18-25, 26-35, 36-45, etc.). 
-- 19. Write a SQL query to find the gender distribution of customers who made purchases in the "Electronics" category.
-- 20. Write a SQL query to find the customers who have increased their spending over time.
-- 21. Write a SQL query to find the customers who have purchased from all available categories.
-- 22. Write a SQL query to find the average number of transactions per customer.
...
spending_trend AS (
	SELECT customer_id,
		ROUND(AVG(CASE WHEN YEAR(sale_date) = 2022 THEN total_spending END),2) AS average_spending_2022,
        ROUND(AVG(CASE WHEN YEAR(sale_date) = 2023 THEN total_spending END),2) AS average_spending_2023
	FROM customer_spending
    GROUP BY customer_id
)
SELECT customer_id
FROM spending_trend
WHERE average_spending_2023 > average_spending_2022;

-- 21. Write a SQL query to find the customers who have purchased from all available categories.

SELECT customer_id, COUNT(DISTINCT category) AS number_of_categories
FROM retail_sales
GROUP BY customer_id
...
SELECT ROUND(AVG(total_transactions), 2) AS average_transactions_per_customer
FROM 	
    (SELECT customer_id,
		COUNT(*) AS total_transactions
	FROM retail_sales
	GROUP BY customer_id) AS customers_transactions;
